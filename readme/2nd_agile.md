...
- **문제점 10**: 게시글을 참조하거나 삭제할 때 해당 게시글의 주소로 이동하지 않는 오류 발생

- 게시글을 참조하거나 삭제할 때, 해당 글들이 pk 값을 가지고 있지 않아 오류가 났다.
<br>→ url 패턴에서 int:pk로 정수형 pk값을 받아 데이터베이스에서 해당 게시글을 검색하게 했다.

게시글을 참조하거나 삭제할 때, 요청을 받을 때 pk 값을 전달하지 않거나 주소에서 pk 값을 명시하지 않아 오류가 발생했습니다.
 URL 패턴에서 int:pk를 사용하여 정수형 pk 값을 받아 데이터베이스에서 해당 게시글을 검색하도록 설정했습니다.
