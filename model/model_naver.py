...
# LSTM 모델 클래스 정의
class LSTMModelnaver(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(LSTMModelnaver, self).__init__()
...
def predict_stock_naver():
    x_train, x_val, x_test, y_train, y_val, y_test, days_2022 = load_data_naver()

    model = LSTMModelnaver(input_size=5, hidden_size=64, output_size=1)
    criterion = nn.L1Loss()
    ...
    #         torch.save(model.state_dict(), 'best_model_naver.pth')
    #         counter = 0
    #     else:
    #         counter += 1
    #         if counter >= patience:
    ...
    model.load_state_dict(torch.load('best_model_naver.pth'))
